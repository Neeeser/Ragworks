...
def test_retrievers_keep_full_filtered_match_order_and_scores() -> None:
    ...
    dense_input = ItemBatch(items=[Item(id="query", text="query", embedding=[0.1, 0.2])])
    lexical_input = ItemBatch(items=[Item(id="query", text="query")])
    output = ItemBatch.from_matches(matches)
    ...
    dense_summary = dense.summarize_io({"items": dense_input}, {"items": output})
    lexical_summary = lexical.summarize_io({"items": lexical_input}, {"items": output})
...
def test_rrf_summary_preserves_each_branch_and_complete_fused_order() -> None:
    branch_one = ItemBatch.from_matches(_matches(["doc:0", "doc:1", "doc:2"]))
    branch_two = ItemBatch.from_matches(_matches(["doc:1", "doc:3"]))
    fused_matches = _matches(["doc:1", "doc:0", "doc:3", "doc:2"])
    fused = ItemBatch.from_matches(fused_matches)
    summary = RRFusionNode(RRFusionConfig()).summarize_io(
        {"items": [branch_one, branch_two]},
        {"items": fused},
    )

    branch_traces = _item_values(summary.inputs)
    assert [_refs(value) for value in branch_traces] == [
        [(item.id, item.score) for item in branch_one.items],
        [(item.id, item.score) for item in branch_two.items],
    ]
...
def test_rrf_summary_records_per_source_rank_contributions() -> None:
    branch_one = ItemBatch.from_matches(_matches(["doc:0", "doc:1", "doc:2"]))
    branch_two = ItemBatch.from_matches(_matches(["doc:1", "doc:3"]))
    fused = ItemBatch.from_matches(_matches(["doc:1", "doc:0"]))
    ...
    assert [(source.source_index, source.rank, source.score, source.contribution) for source in focused.sources] == [
        (0, 2, branch_one.items[1].score, 1 / 62),
        (1, 1, branch_two.items[0].score, 1 / 61),
    ]
...
def test_reranker_summary_keeps_complete_before_and_after_orders() -> None:
    ...
    before = ItemBatch.from_matches(before_matches)
    after = ItemBatch.from_matches(after_matches)
...
def test_ingestion_terminal_preserves_each_branch_and_merged_result() -> None:
    ...
    lexical = ItemBatch.from_chunks(lexical_chunks)
    dense = ItemBatch.from_chunks(dense_chunks)
    node = IngestionOutputNode(IngestionOutputConfig())
    outputs = {"result": node._merge({"items": [lexical, dense]})}

    summary = node.summarize_io({"items": [lexical, dense]}, outputs)
