...
export function mockApi(overrides: Record<string, unknown> = {}) {
  return {
    ...
    previewCollectionDiagnostics: vi.fn(async () => makeDiagnosticsSummary([])),
    ...
    refreshInsights: vi.fn(async () => ({
      snapshot: makeInsightSnapshot(),
      active: null,
      chunk_total: 1,
      can_compute: true,
    })),
    fetchInsightMap: vi.fn(async () => makeInsightMap()),
    fetchInsightGraph: vi.fn(async () => ({
      snapshot: makeInsightSnapshot(),
      documents: makeInsightMap().documents,
      edges: [],
    })),
    ...
    fetchEvalRun: vi.fn(async () => makeEvalRun()),
    fetchEvalRunItems: vi.fn(async () => ({ items: [], document_titles: {} })),
    cancelEvalRun: vi.fn(async () => makeEvalRun({ status: "cancelled" })),
    ...
    fetchToolTemplates: vi.fn(async () => makeToolTemplates()),
    scaffoldToolTemplate: vi.fn(async () => ({ nodes: [], edges: [], viewport: {} })),
