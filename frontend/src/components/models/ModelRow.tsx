...
export interface ModelRowProps {
  ...
  unavailable?: boolean;
  /** Short marker beside the name, e.g. the setup wizard's "Suggested". */
  badge?: ReactNode;
...
export function ModelRow({
  ...
  return (
    ...
    >
      <span className="flex w-full items-center gap-2">
        ...
        <span className="min-w-0 flex-1">
          <span className="flex items-center gap-2">
            <span className="truncate text-ui font-medium text-primary">{model.name}</span>
            {badge}
          </span>
      ...
      {note ? <span className="mt-1 block text-instrument text-data-warn">{note}</span> : null}
    </div>
  );
}
