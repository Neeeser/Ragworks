...
- **Text a node on that path *writes into* items counts against the same
  ...
  fix it.
- **Whose window a writer's budget counts against is decided by its port's
  ...
  carried as `ChunkReach.unchunked` and **accumulated across the path** the way
  the chunker's window accumulates, then compared with the limit on their own:
  two vision nodes in series each fitting the model still leave an item that
  does not, and comparing each budget alone reports nothing about the item that
  actually arrives.
- **Writers are checked against every embedder they reach, not just the one
  binding the chunker's window.** The smallest limit binds the *chunker*, but a
  writer on a branch feeding a different model is absent from that path
  entirely — collecting only the binding reach drops its finding rather than
  reporting it against the wrong model.
...
- **Whether a sampled corpus document needs ingesting is decided by whether it
  ...
  collection.
- **Synthetic question sampling covers every document first, then splits the
  surplus by chunk count, and never plans a document more windows than it has
  chunks** (`allocate_windows` in `app/evals/generation/contexts.py`;
  `_run_plan` accepts at most one question per context window). Pure size
  weighting leaves most of a corpus unasked about, so the dataset cannot say
  how retrieval performs there; pure equal shares measure a manual and a
  one-paragraph note with the same handful of questions, and keep drawing
  windows a small document has already spent — each one a generation call
  whose question the dedup discards. The acceptance cap
  (`acceptance_caps`) is read from that same allocation, per document: a flat
  cap refuses the questions the rota planned.
